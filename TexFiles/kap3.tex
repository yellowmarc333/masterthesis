...
Für die erste Zwischenschicht und die Eingangsschicht berechnen sich Signale analog indem die Eingangssignale $a_{1}^{(k-1)},..., a_{m^{(k-1)}}^{(k-1)}$ durch die Einträge des Datenvektors $x_1, ..., x_F$ ersetzt werden, mit $F$ der Anzahl der Variablen oder der Dimension des Wort-Vektors. Die Aktivierungsfunktion $\sigma$ transformiert die gewichtete Summe dann anschließend und bestimmt wie hoch das finale Signal ist. Jede Schicht außer der Eingangsschicht besitzt eine solche Aktivierungsfunktion. Sie sollte differenzierbar sein, da das Netzwerk im Prozess des Lernens Gradienten berechnet. Dieser Aspekt wird später detailliert erläutert. Es existiert eine Vielzahl an Aktivierungsfunktionen. Es werden im Folgenden Aktivierungsfunktionen vorgestellt, die in dieser Arbeit genutzt werden. Für eine detailliertere Übersicht sei auf \cite{deepEssentials}, S.62 verwiesen. Die \textit{rectified linear unit} (kurz \textit{relu}) hat in den letzten Jahren an Beliebtheit zugenommen. Sei $x$ die gewichtete Summe der Eingangssignale, so ist
...
Dieses Unterkapitel erläutert die Idee der \textit{recurrent neural networks} (kurz \textit{RNN} und motiviert anschließend die darauf basierende \textit{long-short-term-memory neural networks} (kurz \textit{LSTM}). Der Abschnitt basiert größtenteils auf \cite{deepNLP}, S.126-169 (todo updaten).
...
\caption{Bestandteile eines \textit{RNN} Models mit einer \textit{many-to-one} Struktur (\cite{deepL}, S.382)}
\label{abb:RNNArch}
\end{center}
\end{figure}

$x^{(t)}$ ist der $t$-te Teil der Input Sequenz, also das $t$-te Wort. $h^{(t)}$ ist der \textit{hidden state} zum Zeitpunkt $t$ der Sequenz. $o^{(r)}$ ist das Ausgangssignal zum $r$-ten Zeitpunkt, dem Ende der Sequenz.
Die Verbindung zwischen Input und \textit{hidden neurons} wird durch die Gewichtsmatrix $\bm{U}$, die \textit{hidden state} zu \textit{hidden state} Verbindung durch die Gewichtsmatrix $\bm{W}$. Eine weitere Gewichtsmatrix $\bm{V}$ parametrisiert die Verbindung zwischen \textit{hidden state} und Ausgangssignal. Über die wahre Zielvariable $y^{(r)}$, die den 



\begin{itemize}
\item hidden noch gescheit erklären
    \item deepEssentials: S.143-165, bidirectional 159
    \item keras: 174-213 (bidirectional included) (gute quelle)
    \item deepNLP: 119-169, LSTM 138-145
    \item MIT: 373-416, (bidirectional 394-396, LSTM 408- 413)
    \item many to one schöne grafik: deepEssentials S.147
    \item nachteile von traditionellen MLP: deepEssentials S.145
    \item schöne grafiken für lstm: essentials 152, 153
    \item noch schönere grafiken: deepNLP S.140
    \item lstm erhöhen performance: keras 175, zusätzlich 
    \item vanishing gradient lösung: essentials S.150, MIT S.385, 8.2.5, und S. 401
    \item lstm ist nur drop in für RNN: keras 175
    \item struktur mit fully connected/softmax: keras 179 unten
    \item vanishing gradient problem und lösung: keras 184-186, essentials: 149-150
    \item binary classification mit LSTM: keras ab 188
    \item tuning RNNs: deepNLP 138
    \item stoped: deepNLP S. 128, keras:176  MIT 409
    \item Das \textit{RNN} modelliert Vergangene Ereignisse um ein zukünftiges Ereignis vorauszusagen. 
    \item softmax benutzt deepNLP S: 132
    \item sirach: (input plus previous input) can not remember the whole sequence
\end{itemize}{}
